...
function onColumnsChanged(updatedColumns) {
    if (props.actions.toggleColumns?.onReorder) {
        props.actions.toggleColumns.onReorder(updatedColumns);
    } else if (props.actions.toggleColumns?.onChange) {
        // Fallback sur onChange si onReorder n'existe pas
        props.actions.toggleColumns.onChange(updatedColumns);
    }
}
</script>
